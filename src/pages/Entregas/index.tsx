...
const Entregas = () => {
  ...
  const handlePaymentClick = () => {
    navigate('/pagamentos', {
      state: { items, deliveryDetails: form.values, orderId: data?.orderId }
    })
  }
        ...
        const response = await purchase({
          delivery: {
            ...
            address: {
              ...
              city: values.cidade,
              zipCode: values.cep,
              number: parseInt(values.numero, 10),
          ...
          products: items.map((item: { id: any; preco: any }) => ({
            id: item.id,
            price: item.preco
          }))
        }).unwrap()

        navigate('/pagamentos', { state: { orderDetails: response } })
      } catch (error) {
  ...
  const getErrorMessage = (
    ...
    const estadoAlterado = form.touched[fieldName as keyof typeof form.values]
    const estaInvalido = form.errors[fieldName as keyof typeof form.values]
